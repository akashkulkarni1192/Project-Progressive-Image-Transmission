	...
	public void Compress(){
		...
		*/
		...
		while(k<arrayMatrixSize){
			...
			for(int i=0;i<n;i+=2){
				for(int j=0;j<n;j+=2){
					tempMatrix.redMatrix[p][q] = senderMatrix[k].redMatrix[i][j];
					tempMatrix.greenMatrix[p][q] = senderMatrix[k].greenMatrix[i][j];
					tempMatrix.blueMatrix[p][q++] = senderMatrix[k].blueMatrix[i][j];
				}
				p++;
				q=0;
